...
def load_data(dataset, mode):
    ...
    data_file = './'+ dataset +'/'
    dev_compounds = loadNpy(data_file + 'dev_compounds', torch.LongTensor)
    train_compounds = loadNpy(data_file + 'train_compounds', torch.LongTensor)
    test_compounds = loadNpy(data_file + 'test_compounds', torch.LongTensor)
...
def test_classification(dataset):
    ...
    tpr, fpr, _ = precision_recall_curve(labels, predictions)
    PRC = auc(fpr, tpr) #
    ...
    return AUC, precision, recall, roce1, roce2, roce3, roce4, Accuracy, f1, PRC
...
def main(): 
    for epoch in range(args.epoch):
        ...
        if args.mode == 'regression':
            ...
            AUC_test, precision_test, recall_test, roce1, roce2, roce3, roce4, Accuracy, f1, PRC  = test_classification(test_data)   
            print('Epoch:{:03d}'.format(epoch+1),
